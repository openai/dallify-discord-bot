import { AttachmentBuilder, Client, ButtonInteraction, ApplicationCommandOptionType, Embed, EmbedImageData } from "discord.js";
...
export const Save: Action = {
    ...
    run: async (client: Client, interaction: ButtonInteraction) => {
        ...
        await interaction.user.send(`Hello! You wanted me to send you the image(s) I made!\nThe prompt was: ${prompt}`).then(message =>
            interaction.reply({ ephemeral: true, content:'Sending images. Check your DMs!' }).catch(console.error)
        ).catch(error =>
            interaction.reply({ ephemeral: true, content:'You have DMs disabled. I cannot send you the images!'  }).catch(console.error)
        );

        await interaction.user.send({
            files: images.map((img, index) => 
                new AttachmentBuilder(img, {name: createFileName(prompt, index+1)})
            )}
        ).catch(console.error);
    }
};
